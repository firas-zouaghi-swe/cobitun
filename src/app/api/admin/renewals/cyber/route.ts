...
export async function GET(request: NextRequest) {
  ...
  try {
    const renewals = await db.cyberPolicyRenewal.findMany({
      where: { isDeleted: 0 },
    ...
    console.error('Get cyber renewals error:', error);
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const renewal = await db.cyberPolicyRenewal.findUnique({ where: { id } });
    ...
    const updated = await db.cyberPolicyRenewal.update({
      where: { id },
      data: updateData,
      include: {
