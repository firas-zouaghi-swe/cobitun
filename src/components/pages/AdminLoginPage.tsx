...
import { Shield, ArrowLeft, Loader2, Mail, Eye, EyeOff } from 'lucide-react';
...
export default function AdminLoginPage() {
  ...
  const completeLogin = (userData: { id: number; username: string; firstName: string; lastName: string; email?: string | null; role: string; customerId?: number }) => {
    ...
    toast.success(t('auth:toast.welcomeAdmin'));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.user.role !== Roles.ADMIN && data.user.role !== Roles.SUPER_ADMIN) {
        toast.error(t('auth:toast.adminsOnly'));
        return;
      }
      ...
      <Card className="w-full max-w-md shadow-2xl relative z-10 border-tunis-blue/10">
        <CardHeader className="text-center">
          <div className="mx-auto w-16 h-16 bg-gradient-to-br from-tunis-blue to-tunis-blue-dark rounded-2xl flex items-center justify-center mb-4 shadow-lg shadow-tunis-blue/20">
            {mfaState ? <Mail className="w-8 h-8 text-white" /> : <Shield className="w-8 h-8 text-white" />}
          </div>
          <CardTitle className="text-2xl font-bold text-foreground">
            {mfaState ? t('auth:mfa.title') : t('auth:adminLogin.title')}
          </CardTitle>
          <CardDescription>
            {mfaState
        ...
        <CardContent>
          {mfaState ? (
            <form onSubmit={handleOtpVerify} className="space-y-6">
              ...
              <Button type="submit" variant="tunisBlue" className="w-full" disabled={otpLoading || otpCode.length !== 6}>
